...
const PDF = ({ invoice, pdfMode, dataProducts, dataForm, dataIva, dataClass, calculateAmount, handleLineChange, handleRemove, handleAdd, CleanLines, handleMenu, dataAccount }) => {
...
const Text = ({ className, pdfMode, children, color, size, bold }) => {
  return (
    <>
      {pdfMode
        ? (
        ...
        : (
          <TextSpan bold={bold} color={color} size={size} style={compose(`span ${ className ? className : '' }`)}>{children}</TextSpan>
        )}
    </>
  )
}
